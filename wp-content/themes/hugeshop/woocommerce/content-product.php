...
global $product, $woocommerce_loop, $hugeshop_opt, $hugeshop_productrows, $hugeshop_productsfound, $hugeshop_viewmode;
...
<div <?php post_class( $classes ); ?>>
	<div class="product-wrapper">
		...
		<div class="list-col4 <?php if($hugeshop_viewmode=='list-view'){ echo ' col-xs-12 col-sm-4';} ?>">
			<div class="product-image">
				<a href="<?php echo esc_url( get_permalink( $product->id ) ); ?>" title="<?php echo esc_attr( $product->get_title() ); ?>">
					<?php 
					echo ''.$product->get_image('shop_catalog', array('class'=>'primary_image'));
					
					if(isset($hugeshop_opt['second_image'])){
						if($hugeshop_opt['second_image']){
				...
				<div class="buton-layout4 product-layout4">
					<a class="detail-link quickview fa fa-external-link" data-quick-id="<?php the_ID();?>" href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php _e('Quick View', 'hugeshop');?></a>
				</div>
				
				<?php if ( $product->is_on_sale() ) : ?>
					<?php echo apply_filters( 'woocommerce_sale_flash', '<span class="onsale"><span class="sale-bg"></span><span class="sale-text">' . esc_html__( 'Sale', 'hugeshop' ) . '</span></span>', $post, $product ); ?>
				<?php endif; ?>
		...
		<div class="list-col8 <?php if($hugeshop_viewmode=='list-view'){ echo ' col-xs-12 col-sm-8';} ?>">
			<div class="gridview">
				...
				<div class="actions product-layout4">
					<ul class="add-to-links">
						<li class="first"> 
							<?php if ( class_exists( 'YITH_WCWL' ) ) {
								echo preg_replace("/<img[^>]+\>/i", " ", do_shortcode('[yith_wcwl_add_to_wishlist]'));
...
<?php if ( ( ( 0 == $woocommerce_loop['loop'] % 2 || $hugeshop_productsfound == $woocommerce_loop['loop'] ) && $woocommerce_loop['columns'] == 2 )  ) { /* for odd case: $hugeshop_productsfound == $woocommerce_loop['loop'] */
	if($hugeshop_productrows!=1){
		echo '</div>';
	}
} ?>