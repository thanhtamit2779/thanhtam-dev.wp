...
function get_product_as_filter(page) { // Load sản phẩm theo điều kiện              
    ...
    ajax_load({
    	url : url, 
    	data 		: {
			action			: 'search_product_as_region',
            //region_id 		: region_id ,
            city_id 		: city_id ,
    	...
    	jQuery('#filter-product-as-region').append(data)
	    window.busy = false;
    });
...
jQuery(document).ready(function() {	
	...
	jQuery('#btn-search').on('click', function() {
		page = 1 ;
		jQuery('#filter-product-as-region').empty() ;
		get_product_as_filter(1) ;
	}) ;	

	page = 1 ;
	jQuery(window).scroll(function() {
	  var number_page =  jQuery('#filter-product-as-region li:last-child').data('current-page');
	  var total_page  = jQuery('#filter-product-as-region li:last-child').data('total-page');
      if (jQuery(window).scrollTop() + jQuery(window).height() >  jQuery('#filter-product-as-region').height() && !busy) {    		              		
            if(number_page < total_page) {
                busy = true;
                page = page + 1;
                get_product_as_filter(page);
            }           
      }
	});
}) ;
